...
class UserBankAccount(models.Model):
    ...
    def save(self, *args, **kwargs):
        """Prevent changing the account number once set."""
        if self.pk:
            try:
                orig = UserBankAccount.objects.get(pk=self.pk)
            except UserBankAccount.DoesNotExist:
                orig = None
            if orig and orig.account_no != self.account_no:
                raise ValidationError("Account number cannot be changed once set.")
